...
- Al completar Anclas, el servidor encola como notificación interna dos informes para `hola@universosenda.com` y `tanisardella@gmail.com`. Incluyen la dirección de correo de la cuenta Google, el momento profesional opcional, el ranking completo, los puntajes y la devolución orientativa determinística persistida, pero no las 40 respuestas individuales. Los destinatarios pertenecen exclusivamente a la configuración backend: no se muestran como aviso ni requieren un checkbox adicional en el recorrido.
- El formulario posterior mediante `POST /api/contact` queda como una solicitud de contacto opcional y separada; no dispara el informe interno.
