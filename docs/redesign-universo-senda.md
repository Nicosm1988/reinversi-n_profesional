...
- Al completar Anclas, el backend encola automáticamente dos informes internos para las casillas configuradas del equipo. Incluyen la dirección de correo de la cuenta Google, el momento profesional opcional, el ranking completo, los puntajes y la devolución orientativa determinística persistida, pero no identificadores técnicos, fecha exacta ni las 40 respuestas individuales. Este trigger operativo no se presenta como aviso ni como consentimiento adicional en la interfaz; la interpretación por IA permanece separada. El formulario posterior conserva `/api/contact` exclusivamente para una solicitud voluntaria de contacto.
