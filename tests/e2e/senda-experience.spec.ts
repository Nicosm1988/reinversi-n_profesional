...
test("the public experience contains the required journey and no public prices", async ({ page }) => {
  ...
  await expect(page.locator("#procesos article")).toHaveCount(3);
  await expect(page.locator("#como-funciona li")).toHaveCount(4);
  await expect(page.locator("#preguntas details")).toHaveCount(7);
  await expect(page.getByText(/No creemos que una persona sea su currículum/i)).toBeVisible();
...
for (const process of [
  { slug: "orientacion-vocacional", stages: 6 },
  { slug: "reinvencion-profesional", stages: 8 },
  { slug: "transicion-laboral", stages: 8 },
] as const) {
...
test("the initial diagnostic validates each step and preserves navigation", async ({ page }) => {
  ...
  await page
    .getByRole("radio", { name: "Mi trabajo o trayectoria ya no me representa." })
    .check({ force: true });
  await continueButton.click();
